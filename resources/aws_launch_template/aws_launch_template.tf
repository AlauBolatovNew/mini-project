...
echo "<html><h1>${var.autoscaling_group_name}_reviews-api.html</h1></html>" > reviews-api.html
...
echo "RewriteEngine On RewriteRule ^reviews-api$ /reviews-api.html [L] RewriteRule ^reviews$ /reviews.html [L]" | sudo tee /var/www/html/.htaccess
sudo systemctl restart apache2
EOF
  )
